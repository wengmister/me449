...
def TrajectoryGenerator(Tse_initial, Tsc_initial, Tsc_final, Tce_grasp, Tce_standoff, k):
    ...
    packed_1 = mr.ScrewTrajectory(
        Tse_initial, Tse_standoff_init, t_1, t_1/fps, "quintic")
    unpacked_1 = unpack(packed_1, 0)
    for i in unpacked_1:
        traj.append(i)

    # Move from standoff to grasp configuration
    packed_2 = mr.CartesianTrajectory(
        Tse_standoff_init, Tse_grasp_init, t_2, t_2/fps, "quintic")
    unpacked_2 = unpack(packed_2, 0)
    for i in unpacked_2:
        traj.append(i)

    # Close the gripper
    packed_3 = mr.CartesianTrajectory(
        Tse_grasp_init, Tse_grasp_init, t_3, t_3/fps, "quintic")
    unpacked_3 = unpack(packed_3, 1)
    for i in unpacked_3:
        traj.append(i)

    # Move to back to first standoff configuration
    packed_4 = mr.CartesianTrajectory(
        Tse_grasp_init, Tse_standoff_init, t_4, t_4/fps, "quintic")
    unpacked_4 = unpack(packed_4, 1)
    for i in unpacked_4:
        traj.append(i)

    # Move from grasp to final standoff configuration
    packed_5 = mr.ScrewTrajectory(
        Tse_standoff_init, Tse_standoff_final, t_5, t_5/fps, "quintic")
    unpacked_5 = unpack(packed_5, 1)
    for i in unpacked_5:
        traj.append(i)

    # Move from final standoff to final grasp configuration
    packed_6 = mr.CartesianTrajectory(
        Tse_standoff_final, Tse_grasp_final, t_6, t_6/fps, "quintic")
    unpacked_6 = unpack(packed_6, 1)
    for i in unpacked_6:
        traj.append(i)

    # Open the gripper
    packed_7 = mr.CartesianTrajectory(
        Tse_grasp_final, Tse_grasp_final, t_7, t_7/fps, "quintic")
    unpacked_7 = unpack(packed_7, 0)
    for i in unpacked_7:
        traj.append(i)

    # Move back to final standoff configuration
    packed_8 = mr.CartesianTrajectory(
        Tse_grasp_final, Tse_standoff_final, t_8, t_8/fps, "quintic")
...
def main():
    ...
    test = TrajectoryGenerator(
        Tse_initial, Tsc_initial, Tsc_final, Tce_grasp, Tce_standoff, 2)
